...
const truckModels = {
  peterbilt: ['359', '379', '388', '389', '567', '579', '367', '348', '386', '384', '587'],
  kenworth: ['W900', 'T800', 'T680', 'T880', 'T600', 'T660', 'T270', 'T370', 'T440'],
  freightliner: ['Cascadia', 'Century', 'Columbia', 'Coronado', 'FLD120', 'Classic XL', 'Argosy'],
...
const TruckSidebar = ({ isOpen, onClose }) => {
  ...
  // Manejar búsqueda de partes
  const handleSearchParts = () => {
    const searchParams = [];
    if (selectedYear) searchParams.push(`Year: ${selectedYear}`);
    if (selectedMake) searchParams.push(`Make: ${selectedMake.charAt(0).toUpperCase() + selectedMake.slice(1)}`);
    if (selectedModel) searchParams.push(`Model: ${selectedModel.replace(/-/g, ' ').toUpperCase()}`);
    if (selectedCategory) searchParams.push(`Category: ${selectedCategory.replace(/-/g, ' ').toUpperCase()}`);
    ...
    if (searchParams.length > 0) {
      ...
      alert(`Searching for parts:\n\n${searchParams.join('\n')}\n\nRedirecting to parts catalog...`);
      onClose();
    } else {
        ...
        <div className="sidebar-content">
          ...
          <div className="filter-section">
            ...
            >
              ...
              {selectedMake && truckModels[selectedMake] && 
